...
func handleAPIAddArtistCMFunc(w http.ResponseWriter, r *http.Request) {
	...
	team := q.Get("team")
	if team == "" {
		http.Error(w, "URL에 team을 입력해주세요", http.StatusBadRequest)
		return
	}
	name := q.Get("name")
	if name == "" {
	...
	log.Content = fmt.Sprintf("CM 아티스트 ID %s가 추가되었습니다.", a.ID)
