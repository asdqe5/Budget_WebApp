...
// getVFXArtistsFunc 함수는 DB에서 VFX팀의 아티스트를 찾아 반환하는 함수이다.
func getVFXArtistsFunc(client *mongo.Client, sort string, year string) ([]Artist, error) {
...
// getVFXArtistsWithoutRetireeFunc 함수는 퇴사자를 제외한 모든 VFX팀의 아티스트를 찾아 반환하는 함수이다.
func getVFXArtistsWithoutRetireeFunc(client *mongo.Client, sort string, year string) ([]Artist, error) {
	collection := client.Database(*flagDBName).Collection("artists")
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
// rmAllVFXArtists 함수는 DB에서 VFX 아티스트를 모두 삭제하는 함수이다.
func rmAllVFXArtists(client *mongo.Client) error {
	collection := client.Database(*flagDBName).Collection("artists")
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	q := bson.M{"id": primitive.Regex{Pattern: "^(?!cm)", Options: "i"}} // id가 cm으로 시작하지 않으면
