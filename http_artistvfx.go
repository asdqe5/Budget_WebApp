// 프로젝트 결산 프로그램
//
// Description : http VFX 아티스트 관련 스크립트
...
// handleArtistsVFXFunc 함수는 VFX 아티스트 관리 페이지를 띄우는 함수이다.
func handleArtistsVFXFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	...
	if rcp.Resination { // 퇴사자도 함께 보여야하기 때문에 모든 VFX 아티스트들을 가져온다.
		rcp.Artists, err = getVFXArtistsFunc(client, sort, rcp.Year)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	} else { // 퇴사자를 제외한 모든 VFX 아티스트들을 가져온다.
		rcp.Artists, err = getVFXArtistsWithoutRetireeFunc(client, sort, rcp.Year)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	err = genArtistsVFXExcelFunc(rcp.Artists, token.ID) // 엑셀 파일 미리 생성
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	err = TEMPLATES.ExecuteTemplate(w, "artists-vfx", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// handleEditArtistVFXFunc 함수는 VFX팀 아티스트 정보를 수정할 수 있는 페이지를 연다. edit 버튼을 누를 때 실행된다.
func handleEditArtistVFXFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	token, err := getTokenFromHeaderFunc(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
	...
	w.Header().Set("Content-Type", "text/html")
	err = TEMPLATES.ExecuteTemplate(w, "edit-artistvfx", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// handleEditArtistVFXSubmitFunc 함수는 VFX팀 아티스트 정보를 수정하는 페이지에서 UPDATE 버튼을 누르면 작동하는 함수다.
func handleEditArtistVFXSubmitFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	token, err := getTokenFromHeaderFunc(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
	...
	log.Content = fmt.Sprintf("VFX 아티스트 ID %s의 정보가 수정되었습니다.", id)
	...
	http.Redirect(w, r, fmt.Sprintf("/editartistvfx-success?id=%s", id), http.StatusSeeOther)
}

// handleEditArtistVFXSuccessFunc 함수는 VFX팀 아티스트 정보 수정을 성공했다는 페이지를 연다.
func handleEditArtistVFXSuccessFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	token, err := getTokenFromHeaderFunc(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
	...
	w.Header().Set("Content-Type", "text/html")
	err = TEMPLATES.ExecuteTemplate(w, "editartistvfx-success", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// handleUpdateArtistsVFXFunc 함수는 VFX 아티스트를 업데이트하는 페이지를 띄우는 함수이다.
func handleUpdateArtistsVFXFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	token, err := getTokenFromHeaderFunc(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
	...
	w.Header().Set("Content-Type", "text/html")
	err = TEMPLATES.ExecuteTemplate(w, "updateartists-vfx", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// handleArtistsVFXExcelDownloadFunc 함수는 VFX 아티스트 정보를 입력할 엑셀 파일의 템플릿을 생성하여 다운로드하는 함수이다.
func handleArtistsVFXExcelDownloadFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	token, err := getTokenFromHeaderFunc(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
	...
	f.SetCellValue(sheet, "C1", "입사일\n(ex. 2020-01-01)")
	f.SetCellValue(sheet, "D1", "퇴사일\n(ex. 2020-09-01)")
	f.SetCellValue(sheet, "E1", "연봉\n(ex. 2019:2400,2020:2400)")
	f.SetCellValue(sheet, "F1", "동일 연도 연봉 변경\n(ex. 2020-04-01:1920\n변경일과 변경 전 연봉 입력)")
	...
	filename := "vfx_artist_template.xlsx"
...
// handleArtistsVFXExcelSubmitFunc 함수는 엑셀 파일에서 데이터를 가져와 체크하는 페이지로 전달하는 함수이다.
func handleArtistsVFXExcelSubmitFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	token, err := getTokenFromHeaderFunc(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
	...
	if len(fileInfo) != 1 {
		http.Redirect(w, r, "/updateartists-vfx", http.StatusSeeOther)
		return
	}
	...
	for n, line := range excelRows {
		...
		if n == 0 {
			if len(line) != 6 {
				http.Error(w, "엑셀 파일의 Cell 개수는 6개이어야 합니다", http.StatusBadRequest)
				return
			}
			continue
		}
		...
		// Shotgun ID
		artist.ID, err = f.GetCellValue("Sheet1", fmt.Sprintf("A%d", n+1))
		if err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
		...
		a, err := sgGetArtistFunc(artist.ID)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
		...
		// 입사일
		artist.StartDay, err = f.GetCellValue("Sheet1", fmt.Sprintf("C%d", n+1))
		if err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
		...
		// 퇴사일
		artist.EndDay, err = f.GetCellValue("Sheet1", fmt.Sprintf("D%d", n+1))
		if err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
		...
		// 연봉
		salary, err := f.GetCellValue("Sheet1", fmt.Sprintf("E%d", n+1))
		if err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
		...
		// 동일 연도 연봉 변경
		change, err := f.GetCellValue("Sheet1", fmt.Sprintf("F%d", n+1))
		if err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
	...
	err = TEMPLATES.ExecuteTemplate(w, "updateartistsvfx-check", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// handleUpdateArtistsVFXSubmitFunc 함수는 DB의 VFX 아티스트를 업데이트한 후 모두 완료되면 /updateartistsvfx-success로 리다이렉트하는 함수이다.
func handleUpdateArtistsVFXSubmitFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	token, err := getTokenFromHeaderFunc(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
	...
	if len(errArtist) != 0 {
		...
		log.Content = "VFX 아티스트 업데이트에 실패했습니다."
		...
		w.Header().Set("Content-Type", "text/html")
		err = TEMPLATES.ExecuteTemplate(w, "updateartistsvfx-fail", rcp)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	} else {
		log.Content = "VFX 아티스트 업데이트를 완료했습니다."
		...
		http.Redirect(w, r, "/updateartistsvfx-success", http.StatusSeeOther)
	}
}

// handleUpdateArtistsVFXSuccessFunc 함수는 VFX팀 아티스트 정보 업데이트를 성공했다는 페이지를 연다.
func handleUpdateArtistsVFXSuccessFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	token, err := getTokenFromHeaderFunc(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
	...
	w.Header().Set("Content-Type", "text/html")
	err = TEMPLATES.ExecuteTemplate(w, "updateartistsvfx-success", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// genArtistsVFXExcelFunc 함수는 VFX팀의 아티스트 데이터를 엑셀 파일로 생성하는 함수이다.
func genArtistsVFXExcelFunc(artists []Artist, userID string) error {
	path := os.TempDir() + "/budget/" + userID + "/artistsvfx"
	fileName := "vfx_artists.xlsx"
	...
	// 제목 입력
	f.SetCellValue(sheet, "A1", "Shotgun ID")
	f.SetCellValue(sheet, "B1", "부서")
	f.SetCellValue(sheet, "C1", "팀")
	f.SetCellValue(sheet, "D1", "이름")
	f.SetCellValue(sheet, "E1", "입사일")
	f.SetCellValue(sheet, "F1", "퇴사일")
	f.SetCellValue(sheet, "G1", "연봉")
	f.SetCellValue(sheet, "H1", "동일 연도 연봉 변경\n(변경 전 연봉 입력)")
	...
	for n, a := range artists {
		...
		f.SetCellValue(sheet, aPos, a.ID)
		...
		f.SetCellValue(sheet, pos, a.Dept)
		...
		// 입사일
		pos, err = excelize.CoordinatesToCellName(5, n+2) // ex) pos = "E2"
		if err != nil {
			return err
		}
		...
		// Salary
		pos, err = excelize.CoordinatesToCellName(7, n+2) // ex) pos = "G2"
		if err != nil {
			return err
		}
		// 연봉 복호화
		...
		// 동일 연도 연병 변경 정보
		hPos, err := excelize.CoordinatesToCellName(8, n+2) // ex) hPos = "H2"
		if err != nil {
			return err
		}
		// 연봉 복호화
		...
		f.SetCellValue(sheet, hPos, mapToStringFunc(change))

		f.SetCellStyle(sheet, aPos, hPos, style)
	}
...
// handleExportArtistsVFXFunc 함수는 임시 폴더에 저장된 엑셀 파일을 다운로드하는 함수이다.
func handleExportArtistsVFXFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	token, err := getTokenFromHeaderFunc(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
	...
	path := os.TempDir() + "/budget/" + token.ID + "/artistsvfx"
	...
	if len(fileInfo) != 1 {
		http.Redirect(w, r, "/artists-vfx", http.StatusSeeOther)
		return
	}
	...
	log := Log{
		UserID:    token.ID,
		CreatedAt: time.Now(),
		Content:   "VFX 아티스트 페이지에서 아티스트 데이터를 다운로드하였습니다.",
	}
