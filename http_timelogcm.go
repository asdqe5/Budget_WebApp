// 프로젝트 결산 프로그램
//
// Description : http CM 타임로그 관련 스크립트
...
// handleTimelogCMFunc 함수는 CM 타임로그 페이지를 불러오는 함수이다.
func handleTimelogCMFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	...
	// dafult 레벨 미만이면 invalidaccess 페이지로 리다이렉트
	...
	type Recipe struct {
		...
		NoneArtists          []string  // DB에 존재하지 않은 아티스트들의 ID
	...
	for _, timelog := range timelogs {
		...
		if rcp.SelectedTeam != "" {
			if artist.Team != rcp.SelectedTeam { // search한 팀에 속하지 않으면 continue
				continue
			}
		}
	...
	path := os.TempDir() + "/budget/" + token.ID + "/timelogcm/" // json으로 바꾼 타임로그 데이터를 저장할 임시 폴더 경로
	...
	excelFileName := fmt.Sprintf("cm_timelogs_%s", strings.ReplaceAll(date, "-", "_"))
	if rcp.SelectedTeam != "" {
	...
	err = genTimelogCMExcelFunc(rcp.Projects, rcp.ProjectDuration, rcp.TotalDuration, token.ID, excelFileName+".xlsx") // 엑셀 파일 미리 생성
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	err = TEMPLATES.ExecuteTemplate(w, "timelog-cm", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// handleSearchTimelogCMFunc 함수는 CM 타임로그 페이지에서 Search를 눌렀을 때 실행되는 함수이다.
func handleSearchTimelogCMFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	token, err := getTokenFromHeaderFunc(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
	...
	http.Redirect(w, r, fmt.Sprintf("/timelog-cm?date=%s&team=%s&searchword=%s", date, team, searchword), http.StatusSeeOther)
}

// handleUpdateTimelogCMFunc 함수는 CM팀 타임로그를 업데이트하기 위해 엑셀 파일을 임포트하는 페이지를 불러오는 함수이다.
func handleUpdateTimelogCMFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	token, err := getTokenFromHeaderFunc(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
	...
	w.Header().Set("Content-Type", "text/html")
	err = TEMPLATES.ExecuteTemplate(w, "updatetimelog-cm", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// handleTimelogCMExcelDownloadFunc 함수는 CM팀의 타임로그 정보를 입력할 엑셀 파일의 템플릿을 생성하여 다운로드하는 함수이다.
func handleTimelogCMExcelDownloadFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	token, err := getTokenFromHeaderFunc(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
	...
	filename := "cm_timelog_template.xlsx"
...
// handleUploadTimelogCMExcelFunc 함수는 업로드한 엑셀 파일을 임시 폴더로 복사하는 함수이다.
func handleUploadTimelogCMExcelFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	token, err := getTokenFromHeaderFunc(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
...
// handleTimelogCMExcelSubmitFunc 함수는 엑셀 파일에서 데이터를 가져와 체크하는 페이지로 전달하는 함수이다.
func handleTimelogCMExcelSubmitFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	token, err := getTokenFromHeaderFunc(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
	...
	if len(fileInfo) != 1 {
		http.Redirect(w, r, "/updatetimelog-cm", http.StatusSeeOther)
		return
	}
	...
	for n, line := range excelRows {
		...
		for i := 3; i < len(rcp.Projects)+3; i++ {
			...
			if _, exists := rcp.ArtistDatas[cmID]; !exists {
				a := ArtistData{}
				a.Name = name
				a.Timelogs = make(map[string]float64)
				rcp.ArtistDatas[cmID] = a
			}

			// 아티스트의 타임로그 정보 추가
			artistData := rcp.ArtistDatas[cmID]
			artistData.Timelogs[projectName] = duration
			rcp.ArtistDatas[cmID] = artistData
		}
	}
	...
	err = TEMPLATES.ExecuteTemplate(w, "updatetimelogcm-check", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// handleUpdateTimelogCMSubmitFunc 함수는 DB에서 선택한 월에 작성한 CM팀 타임로그 데이터를 업데이트한 후 완료되면 /updatetimelogcm-success로 리다이렉트하는 함수이다.
func handleUpdateTimelogCMSubmitFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	token, err := getTokenFromHeaderFunc(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
	...
	// 3. 내부 인건비와 타임로그 삭제
	// 이미 존재하는 타임로그의 프로젝트 내부 인건비(CM)를 비워주고, DB에 저장하기 전에 CM 타임로그 정보를 삭제한다.
	projects, err := getProjectsByTimelogFunc(year, month, "cm")
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	err = rmCMTimelogFunc(client, year, month)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	if len(errTimelog) != 0 {
		...
		log.Content = fmt.Sprintf("%d년 %d월의 CM 타임로그 임포트 중 타임로그를 추가하지 못했습니다.", year, month)
		...
		w.Header().Set("Content-Type", "text/html")
		err = TEMPLATES.ExecuteTemplate(w, "updatetimelogcm-fail", rcp)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	for _, p := range projectList {
		...
		// VFX 인건비 계산 -> CM 타임로그와 상관없이 변하면 안되기 때문에 저장된 값을 가져온다.
		laborCost.VFX = project.SMMonthlyLaborCost[date].VFX

		// CM 인건비 계산
		cmLaborCost, err := calMonthlyCMLaborCostFunc(p, year, month)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
		laborCost.CM, err = encryptAES256Func(strconv.Itoa(cmLaborCost))
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}

		// RND 인건비 계산 -> CM 타임로그와 상관없이 변하면 안되기 때문에 저장된 값을 가져온다.
		laborCost.RND = project.SMMonthlyLaborCost[date].RND
	...
	if len(errProject) != 0 {
		...
		log.UserID = token.ID
		log.CreatedAt = time.Now()
		log.Content = fmt.Sprintf("%d년 %d월의 CM 타임로그 임포트 중 존재하지않는 프로젝트로 인해 인건비 계산을 못했습니다.", year, month)
		...
		w.Header().Set("Content-Type", "text/html")
		err = TEMPLATES.ExecuteTemplate(w, "updatetimelogcm-projectfail", rcp)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
		...
		log.UserID = token.ID
		log.CreatedAt = time.Now()
		log.Content = fmt.Sprintf("%d년 %d월의 CM 타임로그를 임포트 완료했습니다.", year, month)
		...
		http.Redirect(w, r, fmt.Sprintf("updatetimelogcm-success?date=%s", date), http.StatusSeeOther)
	}
}

// handleUpdateTimelogCMSuccessFunc 함수는 CM팀 타임로그 정보 업데이트를 성공했다는 페이지를 연다.
func handleUpdateTimelogCMSuccessFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	token, err := getTokenFromHeaderFunc(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
	...
	w.Header().Set("Content-Type", "text/html")
	err = TEMPLATES.ExecuteTemplate(w, "updatetimelogcm-success", rcp)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
// genTimelogCMExcelFunc 함수는 CM팀의 타임로그 데이터를 엑셀 파일로 생성하는 함수이다.
func genTimelogCMExcelFunc(projects []string, projectDuration map[string]float64, totalDuration float64, userID string, excelFileName string) error {
	path := os.TempDir() + "/budget/" + userID + "/timelogcm/"
	...
	for _, sortData := range sortList {
		...
		f.SetCellValue(sheet, aPos, strings.Trim(artistID, "cm"))
...
// handleExportTimelogCMFunc 함수는 임시 폴더에 저장한 엑셀 파일을 다운로드하는 함수이다.
func handleExportTimelogCMFunc(w http.ResponseWriter, r *http.Request) {
	// 로그인 상태인지 확인
	token, err := getTokenFromHeaderFunc(w, r)
	if err != nil {
		http.Redirect(w, r, "/signin", http.StatusSeeOther)
	...
	path := os.TempDir() + "/budget/" + token.ID + "/timelogcm"
	...
	ext := filepath.Ext(fileInfo[0].Name())
	if ext != ".xlsx" {
		http.Redirect(w, r, "/timelog-cm", http.StatusSeeOther)
		return
	}
	...
	log := Log{
		UserID:    token.ID,
		CreatedAt: time.Now(),
		Content:   fmt.Sprintf("CM 타임로그 페이지에서 %s년 %s월의 데이터를 다운로드하였습니다.", filename[2], filename[3]),
	}
